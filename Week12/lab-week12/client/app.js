...
const welcomeComponent = {
    template: ` <div class="me">
                    ...
                    <img :src="user.avatar" class="circle row" width="90px">
                    <h5 class="center-align">{{user.name}}</h5>
               </div>`,
    props: ['user']
}
...
socket.on('failed-join', userName => {
    if (userName === app.userName && !app.loggedIn) {
        app.errorMsg = `*Sorry the username <b>${userName}</b> already exists.`
    }
})
